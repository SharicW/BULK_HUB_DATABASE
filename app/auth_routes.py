...
async def auth_me(user: UserOut = Depends(get_current_user)):
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT wallet_address FROM auth_users WHERE id=$1",
            UUID(user.id),
        )
    wallet = row["wallet_address"] if row else None
    return UserOut(id=user.id, email=user.email, wallet_address=wallet)


@router.post("/auth/wallet")
async def auth_set_wallet(body: WalletIn, user: UserOut = Depends(get_current_user)):
    pool = get_pool()
    async with pool.acquire() as conn:
        await conn.execute(
            "UPDATE auth_users SET wallet_address=$1 WHERE id=$2",
            body.wallet_address.strip(),
            UUID(user.id),
        )
    return {"ok": True, "wallet_address": body.wallet_address.strip()}
...
async def get_my_marker(user: UserOut = Depends(get_current_user)):
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow(
            "SELECT user_id, country, city, lat, lng FROM auth_markers WHERE user_id=$1",
            UUID(user.id),
        )
...
async def list_markers(limit: int = 500):
    ...
    async with pool.acquire() as conn:
        rows = await conn.fetch(
            "SELECT user_id, country, city, lat, lng FROM auth_markers ORDER BY updated_at DESC LIMIT $1",
            limit,
        )
