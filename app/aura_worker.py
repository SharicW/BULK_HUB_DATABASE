...
def sync_aura():
    ...
    # total_wallets is computed from len(all_rows) after full fetch for exact count
    total_wallets = 0  # will be set after fetching all rows
    ...
    top_n = all_rows
    total_wallets = len(top_n)  # exact count from actual fetched rows
...
def get_aura_stats():
    ...
    if prev:
        ...
        if time_diff >= 3 * 24 * 3600:  # only compare snapshots at least 3 days apart
...
def get_aura_leaderboard(sort_by: str = "aura", limit: int = 10):
    ...
    order_col = "aura" if sort_by == "aura" else "current_amount"
