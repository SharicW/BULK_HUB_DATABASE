...
def sync_aura():
    ...
    total_wallets = first.get("total_pages", 1) * PAGE_SIZE
    ...
    # Fetch only as many pages as needed to collect STORE_TOP_N rows
    page = 2
    pages_needed = -(-STORE_TOP_N // PAGE_SIZE)  # ceil division
    while page <= pages_needed and len(all_rows) < STORE_TOP_N:
        try:
...
def proxy_wallet(address: str) -> Optional[dict]:
    ...
    # Try dedicated wallet endpoint
    try:
        resp = requests.get(f"{BULK_WALLET_URL}/{address}", headers=REQ_HEADERS, timeout=15)
        logger.info("[aura] Wallet endpoint status %d for %s", resp.status_code, address)
        if resp.status_code == 200:
            data = resp.json()
            logger.info("[aura] Wallet endpoint response keys: %s", list(data.keys()) if isinstance(data, dict) else type(data).__name__)
            # Flat dict with wallet field
            if isinstance(data, dict) and data.get("wallet"):
                return {**data, "source": "api"}
            # List response - take first item
            if isinstance(data, list) and data and data[0].get("wallet"):
                return {**data[0], "source": "api"}
            # Nested under common keys
            for key in ("data", "result", "user", "wallet_data"):
                if isinstance(data, dict) and isinstance(data.get(key), dict) and data[key].get("wallet"):
                    return {**data[key], "source": "api"}
    except Exception as e:
        logger.error("[aura] Wallet endpoint error for %s: %s", address, e)
