...
BULK_XPATHS = {
    ...
    "funding": "//h4[contains(normalize-space(.), 'Funding')]/ancestor::div[contains(@class,'flex') and contains(@class,'flex-col')][1]//div[contains(@class,'items-center')]//span[1]",
    "countdown": "//h4[contains(normalize-space(.), 'Funding')]/ancestor::div[contains(@class,'flex') and contains(@class,'flex-col')][1]//div[contains(@class,'items-center')]//span[2]",
}
...
def _find(driver, xpath: str, timeout: int = 15) -> str:
    """Find element by XPath and return non-empty innerText/textContent."""
    try:
...
def parse_bulk_testnet() -> dict:
    ...
    try:
        ...
        for market in BULK_MARKETS:
            ...
            # Safety: sometimes the label can be captured; keep only real percent value
            if "%" not in funding:
                funding = ""
